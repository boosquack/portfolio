...
> **Project note:** this repo already has the `firecrawl` MCP server configured
> (local scope, `~/.claude.json`, `FIRECRAWL_API_KEY` set — not committed to git).
> That means the tools described below as CLI commands (`firecrawl scrape`, etc.)
> are available here as native MCP tool calls instead — credentials are already
> handled (**Path D is done**). Skip the install/auth steps unless you deliberately
> want the standalone CLI alongside the MCP server. Reach for **Path A** whenever a
> task needs live web data (e.g. auditing a reference site to clone for the
> portfolio redesign — pairs with `.agents/skills/design-taste-frontend/SKILL.md`).
...
In this project, the same operations (`search`, `scrape`, `crawl`, `map`, `extract`, `batch scrape`) are called as MCP tools from the `firecrawl` server rather than as `firecrawl <subcommand>` shell commands — the CLI skill names above map 1:1 to that reasoning, just invoked differently.
...
**Not needed here** — this project already has a real `FIRECRAWL_API_KEY`
configured in the local MCP server config (see project note at the top).
